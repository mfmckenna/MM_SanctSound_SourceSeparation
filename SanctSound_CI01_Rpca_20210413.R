...
#-----------------------------------------------
sdir     = "H:\\RESEARCH\\SanctSound\\data"
...
#-----------------------------------------------
dirSPL  = paste0("H:\\RESEARCH\\SanctSound\\data2\\combineFiles")
...
#-----------------------------------------------
dirSPL    = paste0("H:\\RESEARCH\\SanctSound\\data2\\combineFiles_bySite")
...
dCols
detm = reshape :: melt(cdetft, id.vars = "DateTime", 
                       measure.vars = c("VesselDet_count","bluewhaleDet", "midshipmanDet","bocaccioDet",
                                        "explosionsDet"))
detm$Day = as.Date(detm$DateTime)
detm$DateTime2 = as.POSIXct(detm$DateTime )
# total detections per hour...
#starts at bottom
ulabs = c("Vessel" ,"Whale", "Fish sp","Fish 2", "Explosion" ) #generic
# ulabs = c("Vessel noise" ,"blue whale", "midshipman","bocaccio", "explosion" ) #actualr
...
#add wind and tide
dCols
abioDaym = reshape :: melt(cdetft, id.vars = "DateTime", 
                           measure.vars = c("WSPDms_mean","WaterLevelChange_ft"))
abioDaym$DateTime2 = as.POSIXct(abioDaym$DateTime )
abioDaymWind = abioDaym[ abioDaym$variable == "WSPDms_mean", ]
pWl = ggplot(abioDaymWind, aes(DateTime2, value) )+ 
  ...
  theme_minimal()
abioDaymWind = abioDaym[ abioDaym$variable == "WaterLevelChange_ft", ]
...
#copied to ppt and aligned there
...
#image(1:dim(nvpcaTOL$S)[1],log(hix),as.matrix(nvpcaTOL$S)) #sparse matrix
