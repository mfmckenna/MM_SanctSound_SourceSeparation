...
# o3b <- fread(input="I:/My Drive/share_files/Chronic-Transient_Discrimination/code/NVSPL_Kurt/O3OB_SanctSound.csv")
o3b <- fread(input="I:/My Drive/share_files/Chronic-Transient_Discrimination/code/NVSPL_Kurt/O3OB_SanctSound.csv")
...
rpcaplot <- function(Nv,L,S,kolors){ # packaged code to avoid repetition below, produces a 3 panel plot
  ...
  axis(side=2,at=1:10/11,labels=o3b$CentHz[seq(from=3,to=30,by=3)])
  axis(side=1,at=seq(from=300,to=3300,by=300)/3600,outer=T,labels=seq(from=30,to=270,by=30))
  mtext(text="time in minutes",side=1,outer=T,las=0,line=2.5)
  mtext(text="one-third octave band (Hz)",side=2,outer=T,las=0)
  par(oldpar)
}

nv <- fread(input="I:/Shared drives/Soundscape_Analysis/output_calcAvg/SanctSound_CI02_01/SanctSound_CI02_01_181101_013000_TOL.csv")
